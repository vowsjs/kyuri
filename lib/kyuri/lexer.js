...
Lexer.prototype = {
  ...
  gherkinToken: function (name) {
    ...
    this.token('GHERKIN', name.toUpperCase());
    return true;
  },
  ...
  lineToken: function () {
    ...
    sys.puts(this.chunk.toString());
    sys.puts('Size: ' + indent.length);
    sys.puts('Indents: ' + this.indents);
    ...
    if (size > this.indents) {
      ...
      sys.puts('OUTDENT: ' + (this.indents - size));
      this.outdentToken(this.indents - size);
    }
  ...
  outdentToken: function (outdents) {
    while (outdents > 0) {
      this.token('OUTDENT', '1');
      outdents--; 
    }
    return true;
  },
